...
def merge_qi_value(x_left, x_right, connect_str='~'):
    '''Connect the interval boundary value as a generalized interval and 
    ...
    if isinstance(x_left, (int, float)):
        if x_left == x_right:
            ...
            result = f"{x_left}{connect_str}{x_right}"
...
def covert_to_raw(result, intuitive_order, delimiter='~'):
    ...
    these values back to they raw values.
    """
    covert_result = []
